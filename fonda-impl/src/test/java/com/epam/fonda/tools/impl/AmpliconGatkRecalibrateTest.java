...
class AmpliconGatkRecalibrateTest extends AbstractTest {
    private static final String AMPLICON_GATK_RECALIBRATE_TOOL_TEST_OUTPUT_DATA_PATH =
            "templates/amplicon_gatk_recalibrate_tool_test_output_data.txt";
    ...
    void generate() throws IOException, URISyntaxException {
        AmpliconGatkRecalibrate ampliconGatkRecalibrate = new AmpliconGatkRecalibrate(expectedSample.getTmpOutdir(),
                bamResult);
        Path path = Paths.get(Objects.requireNonNull(this.getClass().getClassLoader()
                .getResource(AMPLICON_GATK_RECALIBRATE_TOOL_TEST_OUTPUT_DATA_PATH)).toURI());
        byte[] fileBytes = Files.readAllBytes(path);
        String expectedCmd = new String(fileBytes);
        bamResult = ampliconGatkRecalibrate.generate(expectedConfiguration, expectedTemplateEngine);
    ...
    private void constructExpectedSample() {
        ...
        expectedSample.setTmpOutdir("tmpOutdir");
    }
